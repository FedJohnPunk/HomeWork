...
internal class WorkerManager
{
    ...
    {
        workerPrinter.PrintArrayOfWorkers(workerRepository.ReadFromFile());
    }
    ...
    private void PrintWorkerById()
    {
        ...
        int workerIndex = workerRepository.FindWorkerIndexById(id);
        if (workerIndex > -1)
        {
            workerPrinter.PrintWorker(workerRepository.GetWorkerByIndex(workerIndex));
        }
        else
        {
    ...
    private void DeleteWorker()
    {
        ...
        if (workerIndex > -1)
        {
            workerRepository.DeleteWorkerByIndex(workerIndex);
            Console.WriteLine("\nДанные удалены.\n");
        }
        else
        {
